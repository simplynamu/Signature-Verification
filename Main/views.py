...
from Main.models import CustomerDetail, VerificationDetail, Attachment
import numpy as np
        ...
        if request.method == 'POST':
            ...
            if 'register' in request.POST:
                ...
                if form.is_valid():
                    form.save()
                    form = CustomerForm()
                    messages.success(request, 'Registration successful! Welcome to our website.')
                ...
                for i in range(0, nof):
                    ...
                    if j.is_valid():
                        VerificationDetail.objects.all().delete()
                        j.save()
                        ...
                        for image in Attachment.objects.filter(customerdetails__c_id=j.cleaned_data['c_id']):
                            ...
                            print('Verification :'+VerificationDetail.objects.get(c_id=j.cleaned_data['c_id']).image.path)
                            vector_database = self.predict(image.file.path)
                            vector_image = self.predict(VerificationDetail.objects.get(c_id=j.cleaned_data['c_id']).image.path)
                            answer = np.sum(np.square(vector_image - vector_database))
                            sum += answer
                              
                        # vector_image = self.predict(VerificationDetail.objects.get(c_id=j.cleaned_data['c_id']).image.path)
                        # vector_database = self.predict(CustomerDetail.objects.get(c_id=j.cleaned_data['c_id']).image.path)
                        # answer = np.sum(np.square(vector_image - vector_database))
